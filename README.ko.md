...
디버깅 전에 알아두시면 좋은 동작이 네 가지 있습니다.

**발신자 번호는 전달하고, 이름은 전달하지 않습니다.** GTBS는 둘 다 제공합니다 —
Incoming Call(`0x2BC1`)과 Call Friendly Name(`0x2BC2`) — 그중 앞의 것만
구독합니다. 같은 주소록을 동기화하는 소프트폰은 번호만 있으면 이름을 스스로
찾습니다. 이름까지 보내면 같은 조회를 두 번 하는 셈이고, 연락처 이름이 SIP
헤더와 다른 기기의 통화기록으로 흘러갑니다. 로컬 조회에는 없는 44바이트 상한도
따라옵니다.

URI는 걸 수 있는 문자열로 줄이거나 버립니다. 스킴을 벗기고 파라미터를 자른 뒤
`0-9 + * # -` 밖의 문자가 있으면 거절합니다. 발신자 정보는 결국 SIP 헤더에
들어가기 때문입니다. 망이나 발신 서버가 표시를 제한한 번호는 URI를 아예 두지
않고, 모듈이 `Anonymous`로 표시합니다.

이름 필드는 패킷 형식에 그대로 남아 있고 비워둘 수 있습니다. 이름을 원하는
배포라면 `0x2BC2`를 구독해 채우면 되고, 프로토콜은 바꾸지 않아도 됩니다. 다만
먼저 확인할 것이 하나 있습니다. GTBS는 E.164(`+8210…`)로 주는데 주소록이
국내 형식(`010-…`)이면, 정규화하지 않는 소프트폰은 어느 쪽으로도 매칭하지
못합니다.
